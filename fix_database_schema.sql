...
-- Ensure unique index on items.name
SELECT COUNT(*) INTO @idx_exists
FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_SCHEMA=@db AND TABLE_NAME='items' AND INDEX_NAME='uniq_items_name';
SET @sql := IF(@idx_exists=0,
  'ALTER TABLE `items` ADD UNIQUE KEY `uniq_items_name` (`name`)',
  'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
